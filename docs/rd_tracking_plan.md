...
#### 1. 代码审查标准 (Code Review)
- [ ] 1RM 算法模块覆盖 Epley 公式边界测试（次数=1时无虚高，次数>15时包含阻尼衰减）；
- [ ] 人体解剖矢量 SVG 组件采用纯声明式渲染，按组数动态映射颜色，无多余 DOM 操作；
- [ ] 桌面大屏布局（$\ge 1024\text{px}$）响应式样式隔离，无水平溢出滚动条。

#### 2. 功能检查清单 (Functional Checklist)
- [ ] 1RM 估算：打卡时依据重量与次数自动计算当前组等效 1RM；
- [ ] PR 突破反馈：历史最好成绩被刷新时，该组高亮展示金色 `🏆 New PR!` 徽章并触发庆祝动效；
- [ ] 肌群热力图：过去 7 天内训练容量自动投射到解剖图（0-4组冷色，10-18组增肌鲜绿，>20组疲劳红）；
- [ ] 桌面看板：大屏幕呈现高密度周历视图，支持拖拽模板排课与周度总容量曲线。
...
| **Gate 2 (数据可视化)** | 1RM 与 SVG 解剖图代码审查通过 | 4 项可视化功能核验通过 | 破纪录与热力图走查通过 | `[ ] 待推进` |
